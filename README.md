...
- **AI assistant in a docked column** — `Ctrl+I` opens it on the right; the column takes real space so the terminal narrows rather than being covered, because half of asking about a server is reading its output. Drag the left edge to resize. A suggested command is **inserted, not run**, and the row above the button names the machine it will go into
