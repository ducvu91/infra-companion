...
### 3D character — a companion that notices when something breaks
...
- **It reacts to your fleet** — a monitoring threshold, replication falling behind or a failed URL check changes its expression, holds a worried posture, and says what happened in a speech bubble. Minimise to the tray and it still reaches you: the character appears on your desktop in a transparent frameless window that **does not steal focus** from whatever you are typing in, and clicking it brings the app back
...
- **Right-click for a radial menu** — expressions, outfits, your pinned Dashboard tools, model switching. Outfits are combinations of visible meshes **you** save and name, because a VRM file carries no notion of an outfit: measured on real models there is only a pile of separate meshes, machine-generated names, or everything merged into one
- **Arms that move like arms** — resting pose and elbows are solved with inverse kinematics against a target beside the hip, so the chain adapts to each model's proportions instead of being tuned for one skeleton. Elbows drift rather than locking, arms lead and trail as the body turns, and tugging the character moves hips and knees too instead of bending it at the waist
